...
age=read.csv('Age structure.csv');
 age0=setDT(age[age$SSP==ssp,c(3:5,85)]);
 setnames(age0,c("X2020","X2100"),c("2020","2100"))
...
age0=age0[,time:=as.numeric(age0$time)]
  
#life_expectancy=merge(age0,life_expectancy[time==2020 | time==2100,],by=c("ISO3","age","time"))
life_expectancy=merge(age0[time==2020,],life_expectancy[time==2020,],by=c("ISO3","age","time"))
...
for (nid in runid) {
  ...
  cscc = NULL
  cohort_scc_income=NULL
  cohort_gap_income=NULL
  
  for (.prtp in prtps) {
    for (.eta in etas) {
      dscc = res_scc[,list(ISO3,model_id,year,gdprate_cc_avg,scc,cgdp,mcc,Income)]
      ...
      cohort_scc=cohort_scc0[,.(prtp=.prtp,eta=.eta,benefit=sum(dscc*income_ratio),cost=sum(dmcc*income_ratio),net=sum(dnet*income_ratio),gdp=sum(dcgdp*income_ratio),pop=Population[1],income=Income[1]),
                             by=c("model_id","ISO3","time","age")]
      
      cohort_scc_income=cohort_scc[,.(prtp=.prtp,eta=.eta,cost=cost*pop,benefit=benefit*pop,tnet=net*pop,tgdp=gdp*pop,pop=pop),by=c("model_id","ISO3","time","age","income")]
      
      cohort_scc_income=cohort_scc_income[,.(prtp=.prtp,eta=.eta,benefit=sum(benefit)/sum(pop),cost=sum(cost)/sum(pop),cgdp=sum(tnet)/sum(tgdp),pgdp=sum(tnet)/sum(pop)),by=c("model_id","time","age","income")]
      
      cohort_gap_income=rbind(cohort_gap_income,cohort_scc_income[,.(prtp=.prtp,eta=.eta,gap=project_gap(.SD)),by=c("model_id","time","income")])
  ...
  lcohort = rbind(lcohort,cohort_scc_income[time==2020,])
  lcohort_gap = rbind(lcohort_gap,cohort_gap_income)
  
}

result_25=lcohort[,lapply(.SD,quantile, probs = c(0.25)),by=c("income","prtp","eta","age","time"),
                  .SDcols=c("cgdp","pgdp")]

result_75=lcohort[,lapply(.SD,quantile, probs = c(0.75)),by=c("income","prtp","eta","age","time"),
                  .SDcols=c("cgdp","pgdp")]

result_median=lcohort[,lapply(.SD,median),by=c("income","prtp","eta","age","time"),
                      .SDcols=c("cgdp","pgdp")]

setnames(result_25,c("cgdp","pgdp"),c("cgdp_25","pgdp_25"))

setnames(result_75,c("cgdp","pgdp"),c("cgdp_75","pgdp_75"))
result=merge(result_median,result_25,by=c("income","prtp","eta","age","time"))
result=merge(result,result_75,by=c("income","prtp","eta","age","time"))

write.csv(result,file=paste0("rich_lag_income","_",.rcp,"_",ssp,".csv"))
 
# result_25=lcohort[,lapply(.SD,quantile, probs = c(0.25)),by=c("income","prtp","eta","age","time"),
#                   .SDcols=c("benefit","cost")]
# 
# setnames(result_25,c("benefit","cost"),c("benefit_25","cost_25"))
# 
# result_75=lcohort[,lapply(.SD,quantile, probs = c(0.75)),by=c("income","prtp","eta","age","time"),
#                   .SDcols=c("benefit","cost")]
# 
# setnames(result_75,c("benefit","cost"),c("benefit_75","cost_75"))
# 
# result_median=lcohort[,lapply(.SD,mean),by=c("income","prtp","eta","age","time"),
#                       .SDcols=c("benefit","cost")]
# 
# result=merge(result_median,result_25,by=c("income","prtp","eta","age","time"))
# 
...
# 
# 
gap_25=lcohort_gap[,lapply(.SD,quantile, probs = c(0.25)),by=c("income","prtp","eta","time"),
                   .SDcols=c("gap")]
setnames(gap_25,c("gap"),c("gap_25"))

gap_75=lcohort_gap[,lapply(.SD,quantile, probs = c(0.75)),by=c("income","prtp","eta","time"),
                   .SDcols=c("gap")]
setnames(gap_75,c("gap"),c("gap_75"))

gap_median=lcohort_gap[,lapply(.SD,median),by=c("income","prtp","eta","time"),
                       .SDcols=c("gap")]
gap=merge(gap_median,gap_25,by=c("income","prtp","eta","time"))

gap=merge(gap,gap_75,by=c("income","prtp","eta","time"))
 
 
# write.csv(result,file=paste0("income_summary","_",.rcp,"_",ssp,".csv"))
write.csv(gap,file=paste0("rich_lag_gap_summary","_",.rcp,"_",ssp,".csv"))
